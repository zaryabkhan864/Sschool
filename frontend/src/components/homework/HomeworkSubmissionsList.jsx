// components/homework/HomeworkSubmissionsList.jsx
...
const GradeRow = ({ submission, homework, onDelete }) => {
  const { t } = useTranslation();
  const [marks, setMarks] = useState(submission.marksObtained ?? "");
  const [feedback, setFeedback] = useState(submission.feedback || "");
  const [expanded, setExpanded] = useState(false);

  const [gradeSubmission, { isLoading: isGrading, isSuccess, error }] = useGradeSubmissionMutation();
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            <div>
              ...
              <input
                ...
                min={0}
                max={homework?.totalMarks || undefined}
              />
            </div>
...
const HomeworkSubmissionsList = () => {
  const { t } = useTranslation();
  const { id } = useParams();

  const { data, isLoading, error, refetch } = useGetSubmissionsForAssignmentQuery(id);
  const [deleteSubmission, { isSuccess: deleteSuccess, error: deleteError }] = useDeleteSubmissionMutation();
  ...
  return (
    <AdminLayout>
      ...
      <div className="max-w-5xl mx-auto">
        <AppPageHeader
          title={homework?.title || t("Submissions")}
          subtitle={t("Due")}
        ...
        <AppCard title={t("Submitted")} icon="fa-inbox" className="mb-6">
          {submissions.length === 0 ? (
            ...
            <div className="space-y-3">
              {submissions.map((s) => (
                <GradeRow key={s._id} submission={s} homework={homework} onDelete={handleDeleteClick} />
              ))}
            </div>
          )}
        </AppCard>
