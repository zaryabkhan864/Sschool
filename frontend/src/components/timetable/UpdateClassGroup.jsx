...
const UpdateClassGroup = () => {
  ...
  useEffect(() => {
    if (groupData) {
      // groupData is the classGroup object (due to transformResponse)
  ...
  const submitHandler = (e) => {
    ...
    // Pass classGroup fields spread at root level (not nested)
  ...
  const gradeOptions = useMemo(
    () =>
      gradesData?.grades?.map((g) => ({
        value: g._id,
        label: `${g.gradeName} (${t("Year")}: ${g.year})`,
      })) || [],
    [gradesData, t]
  );
              ...
              <SearchableDropdown
                ...
                onChange={(val) => {
                  setClassGroup((prev) => ({
                    ...prev,
                    grade: val,
                    courses: [],  // Reset courses when grade changes
                  }));
                  setSelectedCourseOptions([]);
                }}
