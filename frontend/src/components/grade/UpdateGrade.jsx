...
// ✅ CourseDropdown Component (NewGrade se same)
...
// ✅ Main UpdateGrade Component (Updated to match NewGrade theme)
const UpdateGrade = () => {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    if (gradeData) {
      setGrade({
        gradeName: gradeData.grade.gradeName,
        description: gradeData.grade.description,
        courses: gradeData.grade.courses || [],
      });
    }
  ...
  const confirmUpdate = () => {
    ...
    updateGrade({ 
      id: params.id, 
      body: { gradeName, description, courses: courseIds } 
    });
    setShowModal(false);
  };
    ...
    <AdminLayout>
      ...
      <div className="max-w-6xl mx-auto py-4">
        <div className="flex items-center justify-between mb-4 px-2">
          <div>
            <h1 className="text-xl font-bold text-gray-800">{t('Update Grade')}</h1>
            <p className="text-xs text-gray-500">{t('Update existing grade information')}</p>
          </div>
          <button
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-visible">
            ...
            <div className="px-5 py-3 bg-gray-50 border-t border-gray-200 flex justify-end gap-2 rounded-b-xl">
              ...
              <button
                ...
                className={`px-6 py-2 rounded-lg text-xs font-bold text-white transition-all ${updateLoading ? "bg-gray-400" : "bg-blue-600 hover:bg-blue-700 shadow-md"}`}
              >
                {updateLoading ? t('updating') : t('Update Grade')}
              </button>
            </div>
          </div>
        </form>
