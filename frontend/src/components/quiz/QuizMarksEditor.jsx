// components/quiz/QuizMarksEditor.jsx
import React, { useEffect, useMemo, useState } from "react";
import toast from "react-hot-toast";
import { useNavigate, useParams } from "react-router-dom";
import { useTranslation } from "react-i18next";

import { useGetQuizDetailsQuery, useUpdateQuizMarksMutation, useDeleteQuizMutation } from "../../redux/api/quizApi";
...
const QuizMarksEditor = () => {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { id } = useParams();

  const { data, isLoading, error } = useGetQuizDetailsQuery(id);
  const [updateQuizMarks, { isLoading: isSaving, error: saveError, isSuccess }] = useUpdateQuizMarksMutation();
  const [deleteQuiz, { isLoading: isDeleting, isSuccess: deleteSuccess, error: deleteError }] =
  ...
  const questionIndexes = useMemo(
    () => (quiz ? Array.from({ length: quiz.totalQuestions }, (_, i) => i) : []),
    [quiz]
  );

  const handleAnswerChange = (studentId, qIndex, value) => {
    const numeric = Math.max(0, Math.min(quiz.marksPerQuestion, Number(value) || 0));
    setMarks((prev) => ({
      ...prev,
  ...
  const handleSave = () => {
    ...
    updateQuizMarks({ id, marks: marksArray });
  };
  ...
  return (
    <AdminLayout>
      ...
      <div className="max-w-6xl mx-auto">
        <AppPageHeader
          title={quiz.title}
          subtitle={`${quiz.course?.courseName} · ${quiz.classGroup?.displayName}`}
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6 text-sm text-gray-500">
          ...
          <span>
            ...
            {quiz.totalQuestions} {t("questions")} × {quiz.marksPerQuestion} {t("marks")} ={" "}
            {quiz.totalMarks} {t("total")}
          </span>
        </div>
                    ...
                    <tr key={studentId} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-brand-50 text-brand-700">
                          {rowTotal(studentId)} / {quiz.totalMarks}
                        </span>
                      </td>
                    </tr>
                  );
...
export default QuizMarksEditor;
